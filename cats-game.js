...
const stageNodes=
Array.from(document.querySelectorAll(".stage-node"));
...
`${Math.max(
0,
Math.floor(battleState.myHp)
).toLocaleString()} / ${battleState.myMaxHp.toLocaleString()}`;

enemyHpText.textContent=
`${Math.max(
0,
Math.floor(battleState.enemyHp)
...
attacker.attackAnim=
attacker.side==="me"
?UNIT_TYPES[attacker.type].attackTime
:ENEMY_TYPES[attacker.type].attackTime;
...
unit.side==="enemy"
?
STAGES[stageIndex].enemySpeedMul
:1
);
...
battleState.particles.filter(p=>{
...
const attackTime=
u.side==="me"
?UNIT_TYPES[u.type].attackTime
:ENEMY_TYPES[u.type].attackTime;

const lunge=
u.attackAnim>0
?
Math.sin(
(u.attackAnim/attackTime)*
Math.PI
)*10
:0;
...
for(
const p of battleState.particles
){
...
ctx.moveTo(
RIGHT_BASE_X-BASE_ATTACK_RANGE,
GROUND_Y-90
);
...
<div style="font-size:12px;font-weight:400;margin-top:6px">
HP ${ENEMY_TYPES.enemy1.hp}　攻擊 ${ENEMY_TYPES.enemy1.damage}
</div>
...
<div style="font-size:12px;font-weight:400;margin-top:6px">
HP ${ENEMY_TYPES.enemy2.hp}　攻擊 ${ENEMY_TYPES.enemy2.damage}
</div>
