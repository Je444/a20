...
function baseAttack(side,target){
  ...
  if(side==="me"){
    ...
    target.hp-=90;target.hitFlash=.12;createHit({x:RIGHT_BASE_X,y:GROUND_Y},target,true);
  }else{
    battleState.baseCooldownEnemy=BASE_ATTACK_COOLDOWN;
    target.hp-=55;target.hitFlash=.12;createHit({x:LEFT_BASE_X,y:GROUND_Y},target,true);
  }
}
function updateUnit(unit,dt,stageIndex){
  ...
  if(unit.side==="me")unit.x=Math.max(LEFT_BASE_X+80,unit.x-speed*dt);
  else unit.x=Math.min(RIGHT_BASE_X-80,unit.x+speed*dt);
}
function loop(now){
...
function drawBackground(){
  ...
  grad.addColorStop(0,"#1a1c25");grad.addColorStop(.58,"#24252a");grad.addColorStop(1,"#171717");
...
function drawUnit(u){
  ...
  if(img&&img.complete&&img.naturalWidth)ctx.drawImage(img,-size/2,-size/2,size,size);else{ctx.fillStyle=u.side==="me"?"#fff":"#a58bb2";ctx.beginPath();ctx.arc(0,0,u.radius,0,Math.PI*2);ctx.fill()}
  ctx.restore();
